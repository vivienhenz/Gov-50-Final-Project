...
```
It is important to note that I take the log of county population and not population itself because county population in the U.S. varies from hundreds to millions, compressing the data and therefore producing ineffective visualizations. As we can see from the first scatterplot, there is a strong positive correlation between county population and support for regulating CO2 as a pollutant. Furthermore, in the second scatterplot, we can interpret that there is also a somewhat weak positive correlation betzeen county population and AQI scores. From these findings, we can confirm that population is an actual confounder in the inital relationship that we are investigating.
...
```
Here the coefficient of interest is that of the AQI score of -0.016. Causually speaking, this can be interpreted as the following: while controlling for county population, when support for regulating CO2 as a pollutant increases by 1%, the AQI score of the county decreases by 1.6%. This supports my hypothesis that more favorable support for pro-environmental policies actually leads to better air quality in U.S. counties. However, we have to reject this result for the p-value is 0.148, meaning that the probability that this result is due to random chance is 14.8%, rendering it not statistically significant.
